...
class PDFProcessor:
        ...
        try:
            ...
            for page_idx, image in enumerate(images):
                try:
                    ...
                    detection_result = self.layout_detector.detect_layout_and_text(
                        temp_image_path
                    )
                    ...
                    print(
                        f"Grouping {len(layout_blocks)} layout elements on page {page_idx}..."
                    )
                    grouped_figures = self.figure_grouper.group_figure_elements(
                        layout_blocks, page_idx
                    )
                    print(
                        f"Created {len(grouped_figures)} grouped figures from {len(layout_blocks)} elements"
                    )
                    ...
                    for grouped_figure in grouped_figures:
                        try:
                            ...
                            figure_info = {
                                ...
                                "grouping_method": grouped_figure.get(
                            ...
                            enhanced_info = (
                                self.figure_id_generator.generate_figure_info(
                                    grouped_figure, page_idx
                                )
                            )

                            # 기존 정보에 reference_type 추가
                            figure_info["reference_type"] = enhanced_info.get(
            ...
            print(
                f"Mapping {len(result['all_references'])} references to {len(result['figures'])} figures using graph-based approach..."
            )

            try:
                # 이제 figure_mapper가 타입을 고려하여 매핑함
                all_mapped_references = self.figure_mapper.map_references_to_figures(
                    result["all_references"], result["figures"]
                )
                ...
                for page_data in result["pages"]:
                    ...
                    if num_refs > 0 and ref_idx < len(all_mapped_references):
                        page_data["references"] = all_mapped_references[
                            ref_idx : ref_idx + num_refs
                        ]
                        ref_idx += num_refs
                    else:
                ...
                matched_count = sum(
                    1
                    for ref in all_mapped_references
                ...
                result["type_statistics"] = self._calculate_type_statistics(
                    all_mapped_references, result["figures"]
                )

                # 그룹핑 통계 추가
                result["grouping_statistics"] = self._calculate_grouping_statistics(
                    result["figures"]
                )

                print(
                    f"Mapping complete: {matched_count}/{total_refs} references matched ({match_rate:.1%})"
                )

                # 타입별 매칭률 출력
                if "type_statistics" in result:
                    print("\nType-specific matching rates:")
                    for ref_type, stats in (
                        result["type_statistics"].get("matching_by_type", {}).items()
                    ):
                        if stats["total"] > 0:
                            print(
                                f"  {ref_type}: {stats['matched']}/{stats['total']} ({stats['rate']:.1%})"
                            )
                ...
                if "grouping_statistics" in result:
                    ...
                    print(
                        f"  Average elements per figure: {stats['avg_elements_per_figure']:.1f}"
                    )
    ...
    def _calculate_type_statistics(
        self, references: List[Dict[str, Any]], figures: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """타입별 통계 계산"""
        stats = {
            "figures_by_type": {},
        ...
        for fig in figures:
            ...
            stats["figures_by_type"][ref_type] = (
                stats["figures_by_type"].get(ref_type, 0) + 1
            )

        # 참조 타입별 카운트 및 매칭률
        for ref in references:
            ref_type = ref.get("reference_type", "unknown")
            stats["references_by_type"][ref_type] = (
                stats["references_by_type"].get(ref_type, 0) + 1
            )
    ...
    def _calculate_grouping_statistics(
        ...
        for fig in figures:
            ...
            stats["grouping_methods"][method] = (
                stats["grouping_methods"].get(method, 0) + 1
            )
...
def print_type_statistics(result: Dict[str, Any]):
    ...
    for ref_type, data in sorted(stats.get("matching_by_type", {}).items()):
        print(
            f"  {ref_type:12s} {data['matched']:7d} {data['total']:7d} {data['rate']:6.1%}"
        )
