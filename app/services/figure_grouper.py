...
class FigureGrouper:
    ...
    def __init__(self, vertical_threshold: int = 50, horizontal_threshold: int = 100):
        ...
        self.compiled_patterns = [
            (re.compile(p, re.IGNORECASE), t) for p, t in self.id_patterns
        ]
    ...
    def group_figure_elements(
        self, layout_blocks: List[Dict[str, Any]], page_idx: int
    ) -> List[Dict[str, Any]]:
    ...
    def _create_groups(
        ...
        core_elements = [
            (i, e) for i, e in enumerate(elements) if self._is_core_element(e["type"])
        ]
    ...
    def _calculate_element_affinity(
        ...
        if self._is_compatible_element(
            elem2["type"], self._determine_group_type([elem1])
        ):
            scores.append(0.3)
        else:
            scores.append(0.0)
        ...
        # Alignment score
        if elem1["bbox"].is_above(elem2["bbox"]) or elem1["bbox"].is_below(
            elem2["bbox"]
        ):
        ...
        if (
            elem1["element_id"]
            and elem2["element_id"]
            and elem1["element_id"] == elem2["element_id"]
        ):
            scores.append(0.5)

        return sum(scores)

    def _calculate_group_affinity(
        self, elem: Dict[str, Any], group: List[Dict[str, Any]]
    ) -> float:
            ...
            for elem in group:
                ...
                for other_group_id in element_to_groups[elem_id]:
                    if other_group_id != i and other_group_id not in used_groups:
                        ...
                        overlap = len(
                            set(id(e) for e in group) & set(id(e) for e in other_group)
                        )
    ...
    def _is_spatially_related(
        ...
        if abs(bbox1.center_y - bbox2.center_y) < bbox1.height:
            return (
                abs(bbox1.x - bbox2.x2) < self.horizontal_threshold
                or abs(bbox2.x - bbox1.x2) < self.horizontal_threshold
            )
    ...
    def _should_group_elements(
        self, core: Dict[str, Any], elem: Dict[str, Any]
    ) -> bool:
        ...
        if elem_type in ["figure_title", "table_caption"]:
            ...
            if (
                elem_bbox.horizontal_overlap(core_bbox)
                or abs(elem_bbox.center_x - core_bbox.center_x)
            ...
            if (
                elem_bbox.vertical_overlap(core_bbox)
                or abs(elem_bbox.center_y - core_bbox.center_y) < core_bbox.height / 2
            ):
    ...
    def _find_best_group_for_element(
        self, elem: Dict[str, Any], groups: List[List[Dict[str, Any]]]
    ) -> Optional[int]:
            ...
            for member in group:
                ...
                if elem_bbox.is_above(member_bbox) or elem_bbox.is_below(member_bbox):
                    ...
                    alignment_scores.append(
                        max(0, 1.0 - x_diff / self.alignment_threshold)
                    )
    ...
    def _create_figure_from_group(
        self, group: List[Dict[str, Any]], page_idx: int
        ...
        if core_elem["text"] and core_elem not in [
            title_elem,
            caption_elem,
            number_elem,
        ]:
    ...
    def _extract_figure_id(self, group: List[Dict[str, Any]]) -> Tuple[str, str]:
        ...
        core_elem = next(
            (e for e in group if self._is_core_element(e["type"])), group[0]
        )
        return f"{core_elem['bbox'].y}_{core_elem['bbox'].x}", "generated"

    def _extract_text_for_layout(
        self, layout_bbox: Dict[str, int], text_blocks: List[Dict[str, Any]]
    ) -> str:
    ...
    def _is_text_in_region(
        self, text_bbox: Dict[str, int], region_bbox: Dict[str, int]
    ) -> bool:
        ...
        return (
            text_x1 < region_x2
            and text_x2 > region_x1
            and text_y1 < region_y2
            and text_y2 > region_y1
        )
