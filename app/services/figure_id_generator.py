...
class FigureIDGenerator:
        ...
        for patterns in self.typed_id_patterns.values():
            for pattern, mapped_type in patterns:
                self.compiled_patterns.append(
                    (re.compile(pattern, re.IGNORECASE), mapped_type)
                )
    ...
    def generate_figure_info(
        self, layout_block: Dict[str, Any], page_idx: int
    ) -> Dict[str, Any]:
    ...
    def _determine_reference_type(self, layout_type: str, text: str) -> str:
        ...
        if any(keyword in text_lower for keyword in ["fig", "figure", "그림"]):
            ...
            for keyword in ["eq", "equation", "formula", "식", "수식"]
        ):
            return "equation"
    ...
    def _generate_fallback_id(
        self, ref_type: str, page_idx: int, bbox: Dict[str, int]
    ) -> str:
