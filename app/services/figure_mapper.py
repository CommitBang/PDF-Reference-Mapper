...
class FigureMapper:
    ...
    def map_references_to_figures(
        ...
        for i, reference in enumerate(references):
            ...
            if ref_node_id in mappings:
                ...
                mapped_ref["reference_type"] = node_data.get(
    ...
    def _extract_typed_id_from_reference(
        self, ref_text: str
    ) -> Tuple[ReferenceType, Optional[str]]:
    ...
    def _is_type_compatible(
        self, ref_type: ReferenceType, fig_type: ReferenceType
    ) -> bool:
    ...
    def _add_spatial_relationships(self):
        ...
        ref_nodes = [
            n for n in self.graph.nodes() if self.graph.nodes[n]["type"] == "reference"
        ]
        fig_nodes = [
            n for n in self.graph.nodes() if self.graph.nodes[n]["type"] == "figure"
        ]
        ...
        for ref_node in ref_nodes:
            ...
            for fig_node in fig_nodes:
                ...
                # 인접 페이지 관계
                    ...
                    if ref_page > fig_page:
                        self.graph.add_edge(
                            ref_node, fig_node, weight=0.2, relation="next_page_typed"
                        )
    ...
    def _calculate_distance(
        self, bbox1: Dict[str, int], bbox2: Dict[str, int]
    ) -> float:
    ...
    def _create_unmapped_references(
        ...
        try:
            ...
            for node, data in self.graph.nodes(data=True):
                if data["type"] == "reference":
                    ...
                    ref_type_counts[ref_type.value] = (
                        ref_type_counts.get(ref_type.value, 0) + 1
                    )
                elif data["type"] == "figure":
                    fig_type = data.get("fig_type", ReferenceType.UNKNOWN)
                    fig_type_counts[fig_type.value] = (
                        fig_type_counts.get(fig_type.value, 0) + 1
                    )
            ...
            return {
                ...
                "num_references": len(
                    [
                        ...
                        if self.graph.nodes[n]["type"] == "reference"
                    ]
                ),
                "num_figures": len(
                    [
                        n
                        for n in self.graph.nodes()
                        if self.graph.nodes[n]["type"] == "figure"
                    ]
                ),
                "reference_types": ref_type_counts,
                "figure_types": fig_type_counts,
                "avg_degree": (
                    float(np.mean([d for n, d in self.graph.degree()]))
