                    ...
                    if len(box) >= 4:
                        ...
                        for idx, layout_block in enumerate(layout_blocks):
                            if self._boxes_overlap(
                                bbox_formatted, layout_block["bbox"]
                            ):
                                conflicts_with_figure = True
                                layout_blocks[idx]["text"] = (
                                    layout_blocks[idx]["text"] + " " + text
                                )
                                break
                        ...
                        if not conflicts_with_figure and text.strip():
                            text_block = {
                                ...
                                "bbox": bbox_formatted,
                                "confidence": (
                                    float(rec_scores[i]) if i < len(rec_scores) else 1.0
                                ),
                            }
                            text_blocks.append(text_block)

            print(
                f"Created {len(text_blocks)} text blocks and {len(layout_blocks)} layout blocks"
            )

            return {"text_blocks": text_blocks, "layout_blocks": layout_blocks}
