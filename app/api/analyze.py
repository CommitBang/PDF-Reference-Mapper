...
reference_model = api.model(
    "Reference",
    {
        ...
        "not_matched": fields.Boolean(
            description="True if reference not matched to any figure",
            required=False,
            default=False,
        ),
    },
...
page_model = api.model(
    "Page",
    {
        ...
        "page_size": fields.List(
            fields.Integer, description="Page size (width, height)"
        ),
        "blocks": fields.List(
            fields.Nested(text_block_model), description="Text blocks"
        ),
        "references": fields.List(
            fields.Nested(reference_model), description="References"
        ),
    },
)
    ...
    def post(self):
        ...
        try:
            ...
            if not self._validate_file_size(file):
                api.abort(
                    400,
                    f"File size exceeds {Config.MAX_CONTENT_LENGTH // (1024*1024)}MB limit",
                )
            ...
            try:
                # Process PDF - all processing happens in PDFProcessor
                pdf_processor = PDFProcessor(dpi=Config.DPI, use_gpu=Config.OCR_USE_GPU)
                ...
                temp_files = [temp_pdf_path] + result.get("temp_image_paths", [])
